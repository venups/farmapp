...
class TestTripEndpoints:
    @pytest.mark.asyncio
    async def test_create_trip_unauthenticated(self, client: AsyncClient, sample_trip_data):
        response = await client.post("/api/trips/", json=sample_trip_data)
        assert response.status_code in [401, 403]

    @pytest.mark.asyncio
    async def test_create_trip_invalid_dates(self, client: AsyncClient, auth_headers, sample_trip_data):
    ...
    @pytest.mark.asyncio
    async def test_list_trips(self, client: AsyncClient, auth_headers):
        response = await client.get("/api/trips/", headers=auth_headers)
        assert response.status_code == 200

    @pytest.mark.asyncio
    async def test_list_trips_with_pagination(self, client: AsyncClient, auth_headers):
        response = await client.get("/api/trips/?page=1&per_page=5", headers=auth_headers)
        assert response.status_code == 200

    @pytest.mark.asyncio
    async def test_list_trips_with_status_filter(self, client: AsyncClient, auth_headers):
        response = await client.get("/api/trips/?status=planning", headers=auth_headers)
        assert response.status_code == 200

    @pytest.mark.asyncio
    async def test_get_nonexistent_trip(self, client: AsyncClient, auth_headers):
        response = await client.get("/api/trips/000000000000000000000000", headers=auth_headers)
        assert response.status_code == 404

    @pytest.mark.asyncio
    async def test_delete_trip_unauthenticated(self, client: AsyncClient):
