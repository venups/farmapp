...
class TestUserSchemaValidation:
    ...
    def test_invalid_email(self):
        with pytest.raises(ValidationError):
            UserRegister(email="not-email", username="testuser", password="TestPass123", full_name="Test")

    def test_short_password(self):
        with pytest.raises(ValidationError):
            UserRegister(email="test@test.com", username="testuser", password="short", full_name="Test")

    def test_password_no_uppercase(self):
        with pytest.raises(ValidationError):
            UserRegister(email="test@test.com", username="testuser", password="nouppercase1", full_name="Test")

    def test_password_no_number(self):
        with pytest.raises(ValidationError):
            UserRegister(email="test@test.com", username="testuser", password="NoNumber", full_name="Test")
